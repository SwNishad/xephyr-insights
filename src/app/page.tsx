...
export default function Home() {
  ...
  function loadJSONText(text: string) {
    try {
      ...
      if (!isArrayOfObjects(parsed)) {
        alert("Invalid JSON: expected a non-empty array of objects");
        return;
      }
      const rows = parsed;
      const columns = Object.keys(rows[0]);
  ...
  async function loadSampleJSON() {
    try {
      ...
      if (!res.ok) {
        alert(`Sample JSON not found (HTTP ${res.status}). Check public/sample-data/users.json`);
        return;
      }
      const data = await res.json();
      if (!isArrayOfObjects(data)) {
        alert("Sample JSON is not a non-empty array of objects");
        return;
      }
      const rows = data;
      const columns = Object.keys(rows[0]);
      ...
      console.error(e);
      alert("Could not load sample JSON (network/parse error).");
    }
  }
      ...
      <section className="card p-5">
        ...
        <div className="mb-5 grid gap-2">
          ...
          <input
            type="file"
            accept=".csv"
            onChange={(e) => e.target.files && loadCSVFile(e.target.files[0])}
            className="input"
          />
        ...
        <div className="grid gap-2">
          ...
          <textarea
            ...
            placeholder='Example: [{"id":1,"age":22},{"id":2,"age":27}]'
            value={jsonText}
